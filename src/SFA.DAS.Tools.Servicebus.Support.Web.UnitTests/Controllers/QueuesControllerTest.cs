...
public class QueuesControllerTest
{
    ...
    [Test]
    public async Task WhenNoMatchingSessionNamesThenShowsStaleOwnership()
    {
        var responseWithUnknownOwner = new GetMessageCountPerUserQueryResponse
        {
            QueueMessageCount = new Dictionary<string, List<Domain.UserMessageCount>>
            {
                {
                    "TestQueue1",
                    [new UserMessageCount { MessageCount = 5, Queue = "TestQueue1", UserId = "UnknownUser" }]
                }
            }
        };

        var sessionsWithNoMatchingUser = new GetUserSessionsQueryResponse
        {
            UserSessions = new List<UserSession>()
        };

        getMessageCountPerUser.Setup(s => s.Handle(It.IsAny<GetMessageCountPerUserQuery>()))
            .Returns(Task.FromResult(responseWithUnknownOwner));
        getUserSessionsQuery.Setup(s => s.Handle(It.IsAny<GetUserSessionsQuery>()))
            .Returns(Task.FromResult(sessionsWithNoMatchingUser));

        var sut = new QueuesController(getQueuesQuery.Object, getMessageCountPerUser.Object, getUserSessionsQuery.Object);

        var result = await sut.Index(string.Empty, string.Empty, string.Empty, 0, 0, false);
        var queueInfoModel = result.Should().BeOfType<JsonResult>().Which.Value.Should().BeOfType<QueueInformationModel>()
            .Which;

        queueInfoModel.Rows.Single(x => x.Id == "TestQueue1").MessageCountInvestigation.Should()
            .Be("5 (stale ownership)");
    }
}